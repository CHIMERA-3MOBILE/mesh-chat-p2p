import { Injectable } from '@angular/core';
import { AwesomeCordovaNativePlugin, cordova } from '@awesome-cordova-plugins/core';
import { Observable } from 'rxjs';
import * as i0 from "@angular/core";
export class Zeroconf extends AwesomeCordovaNativePlugin {
    getHostname() { return cordova(this, "getHostname", {}, arguments); }
    register(type, domain, name, port, txtRecord) { return cordova(this, "register", {}, arguments); }
    unregister(type, domain, name) { return cordova(this, "unregister", {}, arguments); }
    stop() { return cordova(this, "stop", {}, arguments); }
    watch(type, domain) { return cordova(this, "watch", { "observable": true, "clearFunction": "unwatch", "clearWithArgs": true }, arguments); }
    unwatch(type, domain) { return cordova(this, "unwatch", {}, arguments); }
    close() { return cordova(this, "close", {}, arguments); }
    reInit() { return cordova(this, "reInit", {}, arguments); }
    /**
     * Family of addresses to register: ipv4, ipv6 or any.
     */
    registerAddressFamily;
    /**
     * Family of addresses to watch for: ipv4, ipv6 or any.
     */
    watchAddressFamily;
    static ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "20.3.18", ngImport: i0, type: Zeroconf, deps: null, target: i0.ɵɵFactoryTarget.Injectable });
    static ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "20.3.18", ngImport: i0, type: Zeroconf });
    static pluginName = "Zeroconf";
    static plugin = "cordova-plugin-zeroconf";
    static pluginRef = "cordova.plugins.zeroconf";
    static repo = "https://github.com/becvert/cordova-plugin-zeroconf";
    static platforms = ["Android", "iOS"];
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "20.3.18", ngImport: i0, type: Zeroconf, decorators: [{
            type: Injectable
        }], propDecorators: { getHostname: [], register: [], unregister: [], stop: [], watch: [], unwatch: [], close: [], reInit: [] } });
//# sourceMappingURL=data:application/json;base64,